...
NAME=dovecot
VERSION=2.3.11.3
URL=https://www.dovecot.org/releases/2.3/dovecot-2.3.11.3.tar.gz
...
./configure --prefix=/usr                          \
            --sysconfdir=/etc                      \
            --localstatedir=/var                   \
            --docdir=/usr/share/doc/dovecot-2.3.11.3 \
            --disable-static                       \
...
rm -rf /tmp/rootscript.sh
cat > /tmp/rootscript.sh <<"ENDOFROOTSCRIPT"
cp -rv /usr/share/doc/dovecot-2.3.11.3/example-config/* /etc/dovecot
ENDOFROOTSCRIPT
